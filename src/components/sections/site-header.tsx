  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "bg-black/90 backdrop-blur-sm pt-3" : "bg-transparent pt-3"
      }`}
    >
      ...
      {/* Mobile Menu */}
      {isMobile && isMenuOpen && (
        <div className="fixed inset-0 top-16 bg-black z-40 p-5">
      ...
      <div
        className={`bg-gradient-to-r from-violet-600 to-violet-800 text-white text-xs px-4 py-1.5 text-center font-bold animate-gradient bg-[length:200%_200%] transition-all duration-300 ease-linear  ${
          isScrolled ? "opacity-100" : "opacity-0"
        }`}
      >
        Envíos gratis a todo el país: superando los $110.000 bonificamos el
        envío a cualquier sucursal de Andreani.
      </div>
    </header>
  );
}
